...
const repo = ({ owner, repo }: Partial<IssueOptions>) => `https://github.com/${owner}/${repo}`;
...
export class GitHub {
    ...
    this._github = new GitHubWithPlugins({
      ...
      throttle: {
        onRateLimit: (retryAfter: number, options: any) => {
          this._github.log.warn(`Request quota exhausted for request ${options.method} ${options.url}`);
  ...
  public async getBaseCommitBuild(buildName: string) {
    ...
    if (pr.base.ref !== "master") {
      console.warn(`${pr.html_url} might not have been branched off of master...`);
    }

    console.log(`Attempting to get build for ${repo(this._issueOpts)}/commit/${pr.base.sha}`);
    ...
    const build = statuses.find(status => status.context === `ci/circleci: ${buildName}` && status.state === "success");
    ...
    return parseInt(new URL(build.target_url).pathname.split("/").pop() as string);
  }
